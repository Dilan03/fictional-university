...
class MyNotes {
  ...
  async createNote() {
    ...
    fetch(uniData.root_url + '/wp-json/wp/v2/note/', {
      ...
      document.querySelector("#my-notes").insertAdjacentHTML("afterbegin", `<li data-id="${res.id}" class="fade-in-calc">
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(uniData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.value);
